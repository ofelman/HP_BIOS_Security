...
<#
    ...
    This script will create certificates for use by HP SPM and HP Sure Admin activation
    ...
    arguments:
        ...
        -spm                            # create Secure Platform Management Certificates
        -sureadmin                      # create HP Sure Admin Certificates
...
if ( ($PSBoundParameters.count -eq 0) -or ($help) ) {
    ...
    "`tCreate_BIOS_Certs.ps1 -SPM                  # To create only (certs for) SPM "
    "`tCreate_BIOS_Certs.ps1 -SPM -SureAdmin       # Create all certs for"
    exit
} # if ( ($PSBoundParameters.count -eq 0) -or ($help) )
